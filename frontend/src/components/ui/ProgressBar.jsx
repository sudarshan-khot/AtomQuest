...
export function ProgressBar({ value = 0, size = 'md', showLabel = true, className }) {
  ...
  return (
    <div className={clsx('flex items-center gap-3', className)}>
      <div className={clsx('flex-1 bg-cream-300 rounded-full overflow-hidden', heights[size])}>
        <div
      ...
      {showLabel && (
        <span className="text-xs font-semibold text-graphite-700 w-10 text-right tabular-nums">
          {clamped.toFixed(0)}%
        </span>
      )}
    </div>
